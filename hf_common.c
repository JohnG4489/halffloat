...
bool is_infinity(half_float hf);
bool is_nan(half_float hf);
bool is_zero(half_float hf);
...
uint16_t float_to_half(float f) {
    ...
    uint16_t sign = (f_bits >> 31) & 0x1; // Extraire le bit de signe
    int32_t exp = ((f_bits >> 23) & 0xFF) - 127; // Exposant du float
...
/**
 * @brief Aligne les mantisses et les exposants de deux demi-flottants
 * 
 * @param hf1 Pointeur vers le premier demi-flottant
 * @param hf2 Pointeur vers le second demi-flottant
 */
...
void normalize_and_round(half_float *result) {
    ...
    //On teste le bit de garde
    if (result->mant & 0x10) {
        //Extraire les bits d'arrondi et collants
        uint32_t round_sticky = result->mant & 0xF;

        //round-to-nearest, ties to even
        if (round_sticky > 0x8 || (round_sticky == 0x8 && (result->mant & 0x20))) {
            result->mant += 0x20; //Arrondir
            if (result->mant >= (1 << (HF_MANT_BITS + HF_EXP_BITS + 1))) {
